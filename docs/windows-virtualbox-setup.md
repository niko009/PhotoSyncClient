...
Google по-прежнему используется по умолчанию.
...
хранит оригиналы в структуре `devices/<device>/<album>/<yyyy>/<MM>/...`.
SQLite намеренно не переносится на VirtualBox shared folder.

Если до переключения уже существовали файлы в старом `/data/devices`, их нужно
перенести в `/mnt/server/devices` без изменения относительной структуры. Старый
`bacus-photosync-data` не удалять: в нём остаётся рабочая SQLite-база и он также
служит источником для контролируемой миграции старых оригиналов.
