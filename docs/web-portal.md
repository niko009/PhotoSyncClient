...
Это описание первоначального релиза. Текущий лимит файла в `compose.yml` —
2 GiB; актуальный путь для больших файлов — resumable upload чанками 4 MiB.
...
- корень медиа — `{phone_model}_{user_name}/{folder_name}/` с оригиналами;
...
33 серверных integration/unit теста прошли: изоляция телефонов и пользователей,
...
## Журнал SuperAdmin и серверная консоль

Суперадминистратор после входа попадает в серверную консоль. Журнал показывает
HTTP-запросы (метод, путь без query string, статус, длительность, устройство, IP,
User-Agent, trace ID), ограниченные JSON-тела запроса/ответа и сообщения ILogger
с категорией, уровнем и стеком исключения. Trace ID связывает события одного запроса.
Ответ обработчика ошибки также захватывается. Просмотр журнала не логирует сам себя.
Доступ к `/api/portal/admin/logs` по-прежнему разрешён только политикой SuperAdmin.

Поиск, фильтры источника/уровня и экспорт JSON работают по последним 500 событиям
из 24 последних часовых файлов. Это окно диагностики, не поиск по всему архиву.
Автообновление раз в 5 секунд включается вручную и приостанавливается в скрытой
вкладке или пока раскрыты детали. Журнал управления сохраняется отдельно в БД.

Файлы `requests-*.jsonl` лежат в `PhotoSync:RequestLogging:Directory` (по умолчанию
`logs` в корне приложения). Хранение: `PhotoSync:RequestLogging:RetentionDays`,
по умолчанию 7 дней, допустимо 1–90; очистка выполняется при записи. Настройка
`PhotoSync:RequestLogging:Enabled=false` отключает HTTP-журнал. Сообщения приложения
фильтруются стандартными `Logging:LogLevel` и `Logging:ServerJournal:LogLevel`;
для подробной диагностики можно временно задать последнему `Default=Debug`.
Не включать EF sensitive data logging. Очередь ILogger ограничена 2048 событиями;
при перегрузке вытесняются старые события. Это диагностический журнал, не
гарантированный аудит доставки. Объём диска нужно контролировать отдельно.

Пароли, ключи, cookie и токены не захватываются из заголовков; чувствительные
endpoint исключают тела. В остальных JSON чувствительные поля маскируются
рекурсивно. Невалидные/обрезанные JSON и бинарные тела исключаются. Лимиты тела:
8 КБ запрос, 16 КБ ответ. Сообщения приложения ограничены 16 К символов и маскируют
типичные присваивания секретов; произвольный текст логов всё равно нельзя считать
безопасным местом для секретов. Не писать секреты в ILogger.

Логи ОС, reverse proxy, контейнерного runtime и локальный журнал Android этот
endpoint не импортирует. Исторические записи до включения источника восстановить
невозможно. Изменения не требуют миграции БД, но требуют перезапуска сервера.
