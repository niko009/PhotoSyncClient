...
public sealed class DeviceIsolationTests
{
    ...
    public async Task TwoDevicesCannotSeeOrWriteEachOthersData_AndCanStoreSameHash()
    {
        ...
        Assert.Equal(HttpStatusCode.Unauthorized, (await b.PostAsJsonAsync("/api/devices/register", new RegisterDeviceRequest(uuidA, "Reenrolled", "1"))).StatusCode);
    ...
    [Fact]
    public async Task LegacyUuidCannotBeClaimedWithoutProofOfOwnership()
    {
        await using var factory = new TestPhotoSyncFactory();
        using var client = factory.CreateClient();
