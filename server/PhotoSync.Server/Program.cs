...
app.Use(async (context, next) =>
{
    ...
    context.Response.Headers.ContentSecurityPolicy = "default-src 'self'; script-src 'self' https://accounts.google.com/gsi/client https://analytics.bacus.dev; style-src 'self'; img-src 'self'; connect-src 'self' https://accounts.google.com/gsi/ https://analytics.bacus.dev; frame-src https://accounts.google.com/gsi/; frame-ancestors 'none'; form-action 'self'; base-uri 'none'";
    await next(context);
});
