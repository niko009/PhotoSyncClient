...
async function renderGoogle(clientId) { const google=await waitForGoogle();google.accounts.id.initialize({client_id:clientId,callback:async response=>{message('');try{await api('/google-login',{idToken:response.credential});csrf='';await session();}catch(error){message(error.message);}}});google.accounts.id.renderButton($('google-button'),{theme:'outline',size:'large',shape:'pill',text:'continue_with',width:240}); }
